...
function defaultModelFromProvider(input) {
  const modelParameter = input.parameters.find((parameter) => /(^|[_.-])model([_.-]?id)?$/i.test(parameter.key));
  const modelKey = (modelParameter?.value?.trim() || `${slugify(input.name)}-default`).slice(0, 240);
  const settings = Object.fromEntries(input.parameters
    .filter((parameter) => !/(api[_.-]?key|token|secret|password|credential|auth)/i.test(parameter.key))
    .map((parameter) => [parameter.key, parameter.value]));
  return { modelKey, displayName: (modelParameter?.value?.trim() || `${input.name} Default`).slice(0, 240), settings };
}
...
export async function createProvider(actorUserId, input) {
  ...
  try {
    return await withPlatformAdminContext(actorUserId, async (client) => {
      ...
      const defaultModel = defaultModelFromProvider(input);
      await client.query(
        `INSERT INTO provider_models
          (provider_id, model_key, display_name, status, capabilities, settings, created_by)
         VALUES ($1, $2, $3, $4, '{"autoGenerated":true}'::jsonb, $5::jsonb, $6)`,
        [provider.id, defaultModel.modelKey, defaultModel.displayName,
          input.status === 'connected' ? 'active' : 'inactive', JSON.stringify(defaultModel.settings), actorUserId],
      );
      await client.query(
