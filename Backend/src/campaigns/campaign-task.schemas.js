...
export const listTasksSchema=z.object({source:z.enum(['batch','realtime']).optional(),status:z.enum(['queued','running','paused','completed','failed','busy','no_answer','rejected','unavailable','canceled','manual_follow_up_required','archived']).optional(),search:z.string().trim().max(200).optional(),page:z.coerce.number().int().min(1).default(1),pageSize:z.coerce.number().int().min(1).max(100).default(20)});
