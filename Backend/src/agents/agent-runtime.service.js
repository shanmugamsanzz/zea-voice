...
function buildCompactGroundedSystemPrompt(agent, {
  ...
  const rules = [
    ...
    context.decisionRepair ? `The previous decision was rejected for ${context.decisionRepair.reason}${context.decisionRepair.identifiers?.length ? `; rejected identifiers: ${context.decisionRepair.identifiers.join(', ')}` : ''}. Repair it now: include every required contract key, use only enumerated evidenceIds/responseId values, and provide a non-empty answer when decision is answer. Recheck the complete answer: remove every number, calculated count, ordinal, numbered-list marker, uppercase identifier, acronym, test name or scan name that is not literally present in the cited evidence. For Catalog lists, copy item names exactly from the cited record without adding or expanding terms.` : null,
