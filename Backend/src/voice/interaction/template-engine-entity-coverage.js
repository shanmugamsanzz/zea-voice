...
export async function validateRequestedEntityCoverage(input, invoke) {
  ...
  const completion = await invoke({
    ...
    messages: [{ role: 'system', content: [
      ...
      'Judge actual uncertainty in the caller meaning independently of retrieval ambiguity flags. A clear overview or request to explain previously listed options one by one is not ambiguous simply because it concerns multiple records. Return resolved=true only when evidence covers that intended set; for competing plausible identities return false even if one candidate has detailed facts.',
      ...
      'An unambiguous translation or transliteration of a published name may establish identity across scripts without a literal alias entry. Independently verify equivalence against the original utterance; retrieval rank or a multilingual match label alone is not proof. Similar-sounding alternatives remain unresolved.',
