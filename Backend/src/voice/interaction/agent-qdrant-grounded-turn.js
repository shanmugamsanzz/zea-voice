...
function universalPrompt({ agentPrompt, language, chunks, liveData, previousContext,
  ...
  return [
    ...
    'Use WORKFLOW_ACTION only for a supplied workflow. Follow the configured agent prompt for exactly when a tool should run. Use UPSERT to collect or correct fields for a tool that has configured input fields. For a prompt-driven tool with no configured fields, keep gathering details naturally in the conversation and use EXECUTE only when the configured prompt says its condition has been met.',
    'For every prompt-driven tool execution, put all relevant details that can be extracted from the current question and conversation into argumentsJson. Use EXECUTE only after a complete, unambiguous current caller message authorizes the requested action. A booking, transfer, payment, or other consequential action still needs the caller confirmation required by the configured prompt.',
    'For WORKFLOW_ACTION, return the configured workflowId and toolName, newly supplied or extracted arguments as argumentsJson, and the exact current-caller authorization phrase as authorizationQuote for EXECUTE; otherwise use an empty authorizationQuote.',
