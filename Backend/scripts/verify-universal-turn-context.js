...
assert.deepEqual(context.recentConversation.map(({ content }) => content),
  history.slice(0, -1).map(({ content }) => content));
...
assert.equal(interrupted.recentConversation.length, 1);
...
assert.ok(bounded.recentConversation.length <= 12);
assert.ok(bounded.recentConversation.reduce((sum, turn) => sum + turn.content.length, 0) <= 6000);
...
const result = await runTemplateEngineProductionTurn({
  ...
  retrieveQdrantKnowledge: async (input) => {
    searches += 1;
    assert.equal(input.previousContext[0].content, history[0].content);
    ...
    return { request: createQdrantRetrievalRequest(input), chunks: [],
      diagnostics: { queryEmbeddingCount: 1, qdrantSearchCount: 1,
        returnedChunkCount: 0, maximumChunks: 3, tenantAgentFiltered: true } };
  },
