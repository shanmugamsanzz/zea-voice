...
import { knowledgeSourceUploadError, validateKnowledgeSourceFile } from './knowledgeSourceFile';
import { knowledgeDocumentMetric, type KnowledgeDocumentMetricType } from './knowledgeDocumentMetric';

interface DocumentVersion {
  id: string; documentId: string; versionNumber: number; status: string; isCurrent: boolean;
  checksumSha256: string; sizeBytes: number; pageCount: number | null; chunkCount: number; recordCount: number;
...
export function DocumentVersionPanel({ knowledgeBaseId, document, readOnly, refreshKey, onClose, onUpdated }: {
  ...
  document: { id: string; displayName: string; status: string; documentType: KnowledgeDocumentMetricType };
  readOnly: boolean;
  refreshKey: number;
  onClose: () => void;
  onUpdated: () => void;
  ...
  const chooseReplacement = async (file: File | null) => {
    if (!file) return;
    const validation = await validateKnowledgeSourceFile(file);
  ...
  const uploadReplacement = async () => {
    ...
    } catch (requestError) { setError(knowledgeSourceUploadError(requestError, 'Replacement knowledge file could not be uploaded')); }
  ...
  return <section className="rounded-2xl border border-blue-200 bg-blue-50/30 p-4 sm:p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><span className="text-[10px] font-black uppercase tracking-wider text-blue-600">Immutable Versions</span><h4 className="mt-1 text-base font-bold text-slate-800">{document.displayName}</h4><p className="mt-1 text-[11px] font-medium text-slate-500">Replacement PDF or UTF-8 TXT files create new versions; previous processed versions remain available for rollback.</p></div><div className="flex gap-2"><button type="button" onClick={() => void load()} disabled={loading} className="rounded-lg border border-slate-200 bg-white p-2 text-slate-500 hover:bg-slate-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button><button type="button" onClick={onClose} className="rounded-lg border border-slate-200 bg-white p-2 text-slate-500 hover:bg-slate-50"><X className="h-4 w-4" /></button></div></div>

    {!readOnly && <div className="mt-4 rounded-xl border border-blue-200 bg-white p-4"><div className="flex items-center gap-2"><Upload className="h-4 w-4 text-blue-600" /><span className="text-xs font-bold text-slate-700">Upload replacement PDF or UTF-8 TXT</span></div><p className="mt-1 text-[9px] font-semibold leading-4 text-blue-700">TXT is recommended for Tamil/Tanglish structured content because it preserves exact Unicode text and line breaks.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><label className={`flex min-w-0 flex-1 cursor-pointer items-center rounded-lg border border-dashed border-slate-300 px-3 py-2 text-[10px] font-semibold text-slate-500 hover:border-blue-400 ${documentBusy ? 'pointer-events-none opacity-50' : ''}`}><span className="truncate">{replacement?.name ?? 'Select PDF or UTF-8 TXT (maximum 25 MB)'}</span><input key={`${document.id}-${replacement?.name ?? 'empty'}`} type="file" accept=".pdf,application/pdf,.txt,text/plain" disabled={documentBusy || uploading} className="sr-only" onChange={(event) => { void chooseReplacement(event.target.files?.[0] ?? null); }} /></label><button type="button" onClick={() => void uploadReplacement()} disabled={!replacement || uploading || documentBusy} className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-[10px] font-bold text-white hover:bg-blue-700 disabled:opacity-40">{uploading ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}{uploading ? 'Uploading...' : 'Create New Version'}</button></div>{documentBusy && <p className="mt-2 text-[9px] font-semibold text-amber-700">Wait for the current document job to finish before replacing it.</p>}</div>}
