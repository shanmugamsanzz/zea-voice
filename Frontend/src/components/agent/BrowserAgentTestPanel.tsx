...
export function BrowserAgentTestPanel({ agent, onClose, sessionClient, allowSharing = true }: BrowserAgentTestPanelProps) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-3 backdrop-blur-sm dark:bg-slate-950/60 sm:p-6">
    <div className="browser-agent-test-panel flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-950">
      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_320px]">
        <section className="flex min-h-[440px] flex-col border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 lg:border-b-0 lg:border-r">
          <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 px-5 py-3 dark:border-slate-800">
            ...
            <span className="rounded-full border border-slate-200 bg-slate-100 px-3 py-1 text-[10px] font-bold text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">Runtime: {runtimeState.replaceAll('_', ' ')}</span>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-5">
            {!transcript.length && <div className="flex h-full min-h-64 flex-col items-center justify-center text-center"><AudioLines className="h-10 w-10 text-slate-300 dark:text-slate-700" /><p className="mt-3 text-sm font-bold text-slate-600 dark:text-slate-400">Live transcript will appear here</p><p className="mt-1 text-xs text-slate-400 dark:text-slate-600">Start the test and allow microphone permission.</p></div>}
            {transcript.map((line) => <div key={line.sequenceNumber} className={`flex ${line.speaker === 'agent' ? 'justify-start' : 'justify-end'}`}><div className={`max-w-[84%] rounded-2xl px-4 py-3 ${line.speaker === 'agent' ? 'rounded-tl-none bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-100' : 'rounded-tr-none bg-amber-400 text-slate-950'}`}><div className="mb-1 flex items-center gap-2 text-[9px] font-black uppercase opacity-60"><span>{line.speaker}</span><span>{elapsed(line.offsetMs)}</span></div><p className="text-sm font-semibold leading-relaxed">{line.text}</p></div></div>)}
        ...
        <aside className="min-h-0 space-y-5 overflow-y-auto bg-slate-50 p-5 dark:bg-slate-900/60">
          <div><h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-300"><Clock3 className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />Live metrics</h3><div className="mt-3 grid grid-cols-2 gap-2">{[
          ...
          ].map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:shadow-none"><p className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-600">{label}</p><p className="mt-1 font-mono text-sm font-black text-slate-800 dark:text-slate-200">{value}</p></div>)}</div></div>
          <div><h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-300"><Wrench className="h-4 w-4 text-violet-500 dark:text-violet-400" />Tools and errors</h3><div className="mt-3 space-y-2">{warnings.length ? warnings.map((warning) => <div key={warning.id} className={`rounded-xl border p-3 text-xs font-semibold ${warning.tone === 'error' ? 'border-red-500/30 bg-red-500/10 text-red-700 dark:text-red-200' : 'border-amber-500/30 bg-amber-500/10 text-amber-800 dark:text-amber-100'}`}><div className="flex gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span>{warning.message}</span></div></div>) : <p className="rounded-xl border border-slate-200 bg-white p-4 text-xs font-semibold text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:text-slate-600 dark:shadow-none">No runtime or tool warnings.</p>}</div></div>
