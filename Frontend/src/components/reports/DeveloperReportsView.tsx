    ...
    {selected && <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/45 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDetails(); }}><div className="flex h-full w-full max-w-xl flex-col bg-slate-50 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between border-b border-slate-200 bg-white p-6"><div><p className="text-[10px] font-black uppercase tracking-wider text-amber-500">{selected.transport === 'browser_test' ? 'Browser Test Record' : 'Real Call Record'}</p><h3 className="text-xl font-black text-slate-800">Call Details</h3></div><button onClick={closeDetails} className="rounded-xl border border-slate-200 p-2 text-slate-500"><X className="h-4 w-4" /></button></div>
      <div className="flex-1 space-y-5 overflow-y-auto p-6">{detailsLoading && <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-4 text-xs font-bold text-slate-500"><LoaderCircle className="h-4 w-4 animate-spin text-[#dfa822]" />Loading transcript…</div>}{detailsError && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-bold text-red-700">{detailsError}</div>}
        ...
        {selected.runtimeObservability && <div className="rounded-2xl border border-slate-200 bg-white p-5"><h4 className="text-sm font-black text-slate-800">Runtime Observability</h4><div className="mt-4 grid grid-cols-2 gap-3">{[
          ...
          ['Latest first audio', milliseconds(selected.runtimeObservability.turnLatency.at(-1)?.totalFirstAudioMs)],
          ['Latest retrieval', milliseconds(selected.runtimeObservability.turnLatency.at(-1)?.retrievalMs)],
