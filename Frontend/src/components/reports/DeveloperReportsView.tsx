...
} from './developer-response-trace';
import { DeveloperResponseTracePanel } from './DeveloperResponseTracePanel';
...
interface CallRecord {
  ...
  fromNumber: string;
  toNumber: string;
  transport: 'telephony' | 'browser_test';
  ...
  runtimeObservability: {
    turnLatency: Array<{
      epoch?: number; retrievalMs?: number; llmMs?: number;
      totalFirstAudioMs?: number; finalAnswerFirstAudioMs?: number;
      normalVerifiedRequest?: boolean; firstAudioStatus?: string;
    }>;
    tools: Array<{ name?: string; success?: boolean; durationMs?: number }>;
    providerFailures: Record<string, number> | null;
  } | null;
  aiSummary: {
...
function finiteMetricRange<T extends Record<string, unknown>>(values: T[], field: keyof T) {
  ...
  if (!samples.length) return { minimum: null, maximum: null, average: null };
  return {
...
function answerResponseLatency(turns: NonNullable<CallRecord['runtimeObservability']>['turnLatency']) {
  // Progress acknowledgements and technical recovery audio are not answers.
  return finiteMetricRange(
    turns.filter((turn) => turn.normalVerifiedRequest !== false),
  ...
  return <div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-left">
    ...
    <tbody className="divide-y divide-slate-100 text-xs">{loading
      ...
      >
        ...
        <td className="px-5 py-4"><div className="flex flex-wrap gap-1"><span className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-black uppercase ${call.direction === 'inbound' ? 'border-blue-100 bg-blue-50 text-blue-600' : 'border-amber-100 bg-amber-50 text-amber-600'}`}>{call.direction === 'inbound' ? <PhoneIncoming className="h-3 w-3" /> : <PhoneOutgoing className="h-3 w-3" />}{call.direction}</span>{call.transport === 'browser_test' && <span className="rounded-lg border border-violet-200 bg-violet-50 px-2 py-1 text-[9px] font-black uppercase text-violet-700">Browser test</span>}</div></td>
  ...
  return <div className="zea-call-logs-table-wrap overflow-x-auto"><table className="w-full min-w-[2350px] text-left">
    ...
    <tbody className="divide-y divide-slate-100 text-xs">{loading
      ...
      >
        ...
        <td className="px-4 py-4"><div className="flex flex-col items-start gap-1"><span className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-black uppercase ${call.direction === 'inbound' ? 'border-blue-100 bg-blue-50 text-blue-600' : 'border-amber-100 bg-amber-50 text-amber-600'}`}>{call.direction === 'inbound' ? <PhoneIncoming className="h-3 w-3" /> : <PhoneOutgoing className="h-3 w-3" />}{call.direction}</span>{call.transport === 'browser_test' && <span className="rounded bg-violet-50 px-2 py-1 text-[8px] font-black uppercase text-violet-700">Browser test</span>}</div></td>
...
export function DeveloperReportsView({
  ...
  return <div className="space-y-6">
    ...
    {selected && <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/45 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDetails(); }}><div className="flex h-full w-full max-w-xl flex-col bg-slate-50 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between border-b border-slate-200 bg-white p-6"><div><p className="text-[10px] font-black uppercase tracking-wider text-amber-500">{selected.transport === 'browser_test' ? 'Browser Test Record' : 'Real Call Record'}</p><h3 className="text-xl font-black text-slate-800">Call Details</h3></div><button onClick={closeDetails} className="rounded-xl border border-slate-200 p-2 text-slate-500"><X className="h-4 w-4" /></button></div>
      <div className="flex-1 space-y-5 overflow-y-auto p-6">{detailsLoading && <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-4 text-xs font-bold text-slate-500"><LoaderCircle className="h-4 w-4 animate-spin text-[#dfa822]" />Loading transcript…</div>}{detailsError && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-bold text-red-700">{detailsError}</div>}
        <div className="grid grid-cols-2 gap-3">{[['Agent', selected.agentName || '—'], ['Timestamp', timestamp(selected.startedAt, true)], ['Transport', selected.transport === 'browser_test' ? 'Browser Test' : 'Telephony'], ['Direction', selected.direction.toUpperCase()], ['Outcome', statusLabel[selected.status]], ['Duration', duration(selected.durationSeconds)], ['Sentiment', selected.sentiment || 'Not analyzed']].map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-[9px] font-black uppercase tracking-wider text-slate-400">{label}</p><p className="mt-2 break-words text-xs font-black text-slate-800">{value}</p></div>)}</div>
        <div className="divide-y divide-slate-100 rounded-2xl border border-slate-200 bg-white p-5 text-xs">{[['From', selected.fromNumber], ['To', selected.toNumber], ['Agent ID', selected.agentId || '—'], ['Campaign', selected.campaignName || '—'], ['Provider Call ID', selected.providerCallId || '—'], ['Internal Call ID', selected.id]].map(([label, value]) => <div key={label} className="flex items-start justify-between gap-5 py-3"><span className="shrink-0 font-black uppercase text-slate-400">{label}</span><span className="break-all text-right font-mono font-bold text-slate-700">{value}</span></div>)}</div>
        {selected.runtimeObservability && <div className="rounded-2xl border border-slate-200 bg-white p-5"><h4 className="text-sm font-black text-slate-800">Runtime Observability</h4><div className="mt-4 grid grid-cols-2 gap-3">{[
          ['Measured turns', selected.runtimeObservability.turnLatency.length],
          ['Tool executions', selected.runtimeObservability.tools.length],
          ['Latest response latency', milliseconds(latestFiniteMetric(selected.runtimeObservability.turnLatency.filter((turn) => turn.normalVerifiedRequest !== false), 'finalAnswerFirstAudioMs'))],
          ['Latest retrieval', milliseconds(latestFiniteMetric(selected.runtimeObservability.turnLatency, 'retrievalMs'))],
          ['Lowest response latency', milliseconds(answerResponseLatency(selected.runtimeObservability.turnLatency).minimum)],
          ['Highest response latency', milliseconds(answerResponseLatency(selected.runtimeObservability.turnLatency).maximum)],
          ['Average response latency', milliseconds(answerResponseLatency(selected.runtimeObservability.turnLatency).average)],
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5"><div className="mb-4 flex items-center justify-between"><h4 className="text-sm font-black text-slate-800">Transcript</h4><span className="rounded-full bg-slate-100 px-2 py-1 text-[9px] font-black uppercase text-slate-500">{selected.transcript?.length ?? 0} entries</span></div>{selected.transcript?.length ? <div className="space-y-4">{selected.transcript.map((entry) => <TranscriptMessage key={entry.id} entry={entry} showDeveloperDiagnostics={canViewDeveloperResponseTrace(role)} />)}</div> : <p className="py-8 text-center text-xs font-semibold text-slate-400">No finalized transcript entries were saved for this call.</p>}</div>
